...
public class MinimumSpanningTreeUtil {
    ...
    public static <V, E extends Weighted> PositionalList<Edge<E>> kruskal(Graph<V, E> g) {
        ...
        Map<Vertex<V>, Position<Vertex<V>>> pqEntries = new LinearProbingHashMap<>();
        
        // insert the edges into the priority queue, partially ordered to edge weight
        ...
        while (components > 1) { // process edges in order of increasing weight
            ...
            Vertex<V> u = d.find(g.endVertices(e)[0]).getElement();
            Vertex<V> v = d.find(g.endVertices(e)[1]).getElement();
            if (u != v) {
                d.union(pqEntries.get(u), pqEntries.get(v));
                t.addLast(e);
                components--;
            }
        }
